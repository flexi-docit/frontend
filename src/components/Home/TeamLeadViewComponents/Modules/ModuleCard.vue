    ...
    <section class="module-card-options">
      ...
      <span class="module-card-options-tags-grp">
        <p
          ...
          :key="tag.id"
        >
          {{ tag.name.length > 5 ? tag.name.substr(0, 5) + "..." : tag.name }}
        </p>
      </span>
    </section>
  </div>
  ...
  methods: {
    ...
    async deleteModule() {
      ...
      const confirmation = window.confirm(
        "Careful! You are about to delete a module, this is a dangerous action"
      );
      ...
      if (confirmation) {
        ...
        if (deleteModuleRes.status) {
          this.$parent.$emit("deleteModule", this.module.id);
