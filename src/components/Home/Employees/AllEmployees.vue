<template>
  <section class="team-lead-employees">
    <h1 class="team-lead-employees-header">
    ...
    <div class="team-lead-employees-container">
      <div
        v-for="emp in shownEmployees"
        :key="emp.id"
        class="team-lead-employee-row"
      >
        <p class="name">
          {{ emp.name }}
        </p>
  ...
  methods: {
    searchEmployee() {
      ...
      this.allEmployees.forEach((emp) => {
        if (
          employeeRegex.test(emp.name) ||
          employeeRegex.test(emp.email) ||
          employeeRegex.test(emp.role)
        )
          list.push(emp);
      });
