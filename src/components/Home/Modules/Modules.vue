<template>
    <section class="team-lead-modules" :style="topStyle">
        <h1 class="team-lead-modules-header">Modules | {{ projectName }}</h1>
        <div class="team-lead-modules-search">
            <input placeholder="Search" @input="searchModule" v-model="searchTerm" />
            <span>&#8981;</span>
            <button @click="openModuleCreationModal">+</button>
        </div>
        ...
        <div class="team-lead-modules-btn-grp">
            <button v-if="Math.ceil(shownModules.length / 6) > 0" @click="previousModulesPage">&lt;</button>
            <button v-for="num in Math.ceil(shownModules.length / 6)" :class="num === modulePageNumber ? 'btn-active' : ''"
                @click="changeModulesPage(num)">{{
                    num }}</button>
            <button v-if="Math.ceil(shownModules.length / 6) > 0" @click="nextModulesPage">&gt;</button>
        </div>
    </section>
</template>
...
export default {
    data() {
        return {
            ...
            topStyle: { backgroundImage: `url(${topbg})`, backgroundRepeat: 'no-repeat', backgroundPosition: 'center center', backgroundSize: 'cover' },
        }
    },
    props: {
        modules: Array
    ...
    created() {
        ...
        this.shownModules.sort((a, b) => { return (a.createdAt >= b.createdAt) ? 1 : -1 })
    }
}
</script>
