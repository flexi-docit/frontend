    ...
    >
      ...
      >
        <option
          ...
          :key="idx"
          :value="employee.id"
          :selected="employee.id == editingModule.lead_id"
        >
          {{ employee.name }}
        </option>
      </select>
...
export default {
  props: {
    editingModule: { type: Object, default: null },
    allEmployees: {
      type: Array,
      default() {
        return [];
    ...
    async submitEditModule() {
      ...
      try {
        ...
        if (editModuleRes.status) {
          ...
          const alertMessage =
            'Module "' + this.editingModule.name + '" edited!';
          alert(alertMessage);

          this.$parent.$emit("closeModuleModal");
        } else {
          if (editModuleResponse.status === 401) {
